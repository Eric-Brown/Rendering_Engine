...
Model::Model(std::vector<Vertex> &preloadedMesh, std::vector<uint32_t> preloadedMeshIndices)
		: mesh(std::move(preloadedMesh)), meshIndexes(std::move(preloadedMeshIndices)) {
}
...
void Model::processSceneObject(const aiScene *scene) {
	...
	if (pAiMesh->HasTextureCoords(0)) {
		...
		cout << "There are " << pAiMesh->GetNumUVChannels() << " UV channels" << endl;
	}
	if (pAiMesh->HasVertexColors(0)) {
		cout << "The given mesh has vertex colors" << endl;
	}
	cout << "Going to read in: " << pAiMesh->mNumVertices << " amount of vertices." << endl;
	...
	cout << "There are now: " << meshIndexes.size() << " number of indices" << endl;
